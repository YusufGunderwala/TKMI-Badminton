...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        flash_set('players', 'Invalid security token.', 'error');
    ...
    if (empty($its) || empty($full) || empty($disp) || empty($moh) || empty($gen) || empty($wa)) {
        flash_set('players', 'All fields except photo are required.', 'error');
        header('Location: add.php');
        exit;
    }
    if (!in_array($gen, ['Boys','Girls'])) {
        flash_set('players', 'Invalid gender selection.', 'error');
        header('Location: add.php');
        exit;
    }
    ...
    if (!empty($_FILES['photo']['name'])) {
        $photoName = uploadImage($_FILES['photo'], __DIR__ . '/../../uploads/players', 'p_' . $its);
        if ($photoName === false) {
            flash_set('players', 'Failed to upload photo.', 'error');
            header('Location: add.php');
            exit;
        }
    }
    ...
    try {
        $stmt = db()->prepare('INSERT INTO players (its_id, full_name, display_name, mohallah, gender, whatsapp, photo_path) VALUES (?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$its, $full, $disp, $moh, $gen, $wa, $photoName]);
        AppCache::flush();
        flash_set('players', 'Player added successfully!', 'success');
        header('Location: index.php');
        exit;
    } catch (PDOException $e) {
        // Unique constraint violation (ITS ID)
        if ($e->getCode() == 23505) {
            flash_set('players', 'A player with this ITS ID is already registered.', 'error');
        } else {
            flash_set('players', 'Database error: ' . $e->getMessage(), 'error');
        }
        if ($photoName) deleteUpload(__DIR__ . '/../../uploads/players/' . $photoName);
        header('Location: add.php');
        exit;
    }
}
...
<div class="mb-8">
    ...
    <h2 class="text-3xl font-black font-display text-[#0f2044]">Add New Player</h2>
</div>

<?= flash_html('players') ?>

<!-- Alpine Container for Form & Image Dropzone & Badminton Smash Animation -->
<div x-data="playerFormHandler()" class="space-y-6">

    <div class="bg-white border border-slate-200 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden max-w-3xl">
        <form action="add.php" method="POST" enctype="multipart/form-data" @submit="handleSubmit($event)" class="p-8" id="playerForm">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-full md:col-span-1">
                    ...
                    <input type="text" name="its_id" required pattern="[0-9]{8}" maxlength="8" placeholder="e.g. 12345678" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors font-mono">
                ...
                <div class="col-span-full">
                    ...
                    <input type="text" name="full_name" required placeholder="e.g. Hussain Ali" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors">
                </div>

                <div class="col-span-full md:col-span-1">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Display Name <span class="text-red-500">*</span></label>
                    <input type="text" name="display_name" required placeholder="e.g. Hussain A." class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors">
                ...
                <div class="col-span-full md:col-span-1">
                    ...
                    <input type="text" name="whatsapp" required placeholder="e.g. +91 9876543210" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors">
                ...
                <div class="col-span-full">
                    ...
                    <input type="text" name="mohallah" required placeholder="e.g. Saifee Park" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors">
                </div>
                ...
                <!-- ============================================================ -->
                <div class="col-span-full">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Player Photo (Optional)</label>
                    ...
                    <!-- Error Alert State (Inside dropzone context) -->
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="text-left">
                                    ...
                                    <p class="text-[11px] text-emerald-600 font-bold mt-1 flex items-center gap-1">
                                        <i class="ph-bold ph-check-circle"></i> Valid image preview loaded
                                    </p>
                                </div>
                            </div>
    ...
    <div x-show="isUploading" 
        ...
        <div class="bg-gradient-to-b from-[#0a1733] to-[#060e22] border-2 border-[#c9a84c]/40 rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-[0_30px_90px_rgba(0,0,0,0.9)] text-center relative overflow-hidden ring-1 ring-white/20">
            ...
            <div class="relative w-full h-56 sm:h-64 rounded-2xl overflow-hidden shadow-2xl border border-white/15 bg-[#030914] mb-5">
                <canvas id="playerSmashCanvas" class="w-full h-full block"></canvas>
            ...
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#c9a84c]/20 border border-[#c9a84c]/50 text-[#c9a84c] text-xs font-black uppercase tracking-widest mb-2 shadow-sm">
                ...
                <span x-text="smashStatus">SMASHING UPLOAD...</span>
            </div>

            <!-- High-Contrast Title & Subtitle -->
            <h3 class="text-2xl sm:text-3xl font-black font-display text-white mb-1.5 tracking-tight drop-shadow-sm">Processing Player Profile</h3>
            <p class="text-xs sm:text-sm text-slate-300 mb-5 font-medium max-w-md mx-auto leading-relaxed">Uploading avatar, validating Dawoodi Bohra ITS ID, and registering player into the tournament roster.</p>
...
function playerFormHandler() {
    return {
        ...
        handleSubmit(event) {
            ...
            this.$nextTick(() => {
                setTimeout(() => {
                    if (typeof BadmintonSmashEngine !== 'undefined') {
                        this.smashEngine = new BadmintonSmashEngine('playerSmashCanvas');
            ...
            const statuses = [
                'SMASHING PHOTO UPLOAD...',
                'OPTIMIZING RESOLUTION...',
                'REGISTERING IN TOURNAMENT ROSTER...',
                'FINALIZING COMMUNITY RECORDS...'
            ];
            let statusIdx = 0;
