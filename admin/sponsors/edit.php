<?php
// ============================================================
// Sponsors - Edit (with Live Preview & 3D Smash Animation)
// ============================================================
...
if ($sponsorId <= 0) {
    flash_set('sponsor', 'Invalid sponsor ID.', 'error');
...
if (!$sponsor) {
    flash_set('sponsor', 'Sponsor not found.', 'error');
    header('Location: index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        flash_set('sponsor_edit', 'Invalid security token.', 'error');
        header('Location: edit.php?id=' . $sponsorId);
        exit;
    }
    ...
    if (empty($name)) {
        flash_set('sponsor_edit', 'Sponsor name is required.', 'error');
        header('Location: edit.php?id=' . $sponsorId);
        exit;
    }

    $photoName = $sponsor['image_path'];

    // Handle New Logo Upload
    if (!empty($_FILES['logo']['name'])) {
        $uploadedLogo = uploadImage($_FILES['logo'], __DIR__ . '/../../uploads/sponsors', 'sp_' . time());
        if ($uploadedLogo === false) {
            flash_set('sponsor_edit', 'Failed to upload logo. Ensure file is PNG, SVG, or JPG under 2MB.', 'error');
            header('Location: edit.php?id=' . $sponsorId);
            exit;
        }
        // Remove old logo
        if (!empty($sponsor['image_path']) && $sponsor['image_path'] !== $uploadedLogo) {
            deleteUpload(__DIR__ . '/../../uploads/sponsors/' . $sponsor['image_path']);
        }
        $photoName = $uploadedLogo;
    }

    try {
        $stmt = $pdo->prepare('UPDATE sponsors SET name = ?, image_path = ? WHERE id = ?');
        $stmt->execute([$name, $photoName, $sponsorId]);
        AppCache::flush();
        flash_set('sponsor', "Sponsor \"{$name}\" updated successfully!", 'success');
        header('Location: index.php');
        exit;
    } catch (PDOException $e) {
        flash_set('sponsor_edit', 'Database error: ' . $e->getMessage(), 'error');
        header('Location: edit.php?id=' . $sponsorId);
        exit;
    }
}
...
<div class="mb-8">
    ...
    <h2 class="text-3xl font-black font-display text-[#0f2044]">Edit Sponsor</h2>
</div>

<?= flash_html('sponsor_edit') ?>

<!-- Alpine Container with Live Preview & 3D Smash Animation -->
<div x-data="sponsorEditHandler()" class="space-y-6">

    <div class="bg-white border border-slate-200 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden max-w-2xl">
        <form action="edit.php?id=<?= $sponsorId ?>" method="POST" enctype="multipart/form-data" @submit="handleSubmit($event)" class="p-8" id="sponsorEditForm">
            <?= csrf_field() ?>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" required value="<?= e($sponsor['name']) ?>" placeholder="e.g. Al-Manar Travels" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-3 transition-colors">
                </div>

                <!-- ============================================================ -->
                <!-- INTERACTIVE DROPZONE WITH LIVE PREVIEW & LOGO REPLACEMENT    -->
                <!-- ============================================================ -->
                <div>
                    ...
                    <div @dragover.prevent="isDragging = true"
                         ...
                         :class="{
                             ...
                             'border-emerald-300 bg-emerald-50/20': (imagePreview || existingLogo) && !errorMessage,
                             'border-slate-200 bg-slate-50/50 hover:bg-slate-100/70 hover:border-slate-300': !isDragging && !imagePreview && !existingLogo && !errorMessage
                         }"
                         class="relative border-2 border-dashed rounded-3xl p-6 transition-all duration-200 text-center">
                        
                        <!-- State A: Existing Logo on Server -->
                        <div x-show="existingLogo && !imagePreview" class="flex flex-col sm:flex-row items-center justify-between gap-4 p-2">
                            <div class="flex items-center gap-4">
                                <div class="relative w-20 h-20 rounded-2xl overflow-hidden shadow-md border-2 border-white bg-white p-2 flex items-center justify-center flex-shrink-0">
                                    <img :src="existingLogo" alt="Current Logo" class="max-w-full max-h-full object-contain">
                                </div>
                                <div class="text-left">
                                    <div class="flex items-center gap-2">
                                        <span class="text-sm font-black text-slate-900">Current Logo</span>
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-blue-100 text-blue-800">Active</span>
                                    </div>
                                    <p class="text-xs text-slate-400 mt-0.5 font-medium">Displayed in footer and scoreboards</p>
                                </div>
                            </div>
                            ...
                            <button type="button" 
                                ...
                                <i class="ph-bold ph-upload-simple"></i> Replace Logo
                            </button>
                        </div>

                        <!-- State B: New Selected File Preview -->
                        <div x-show="imagePreview" x-transition.opacity.duration.300ms class="flex flex-col sm:flex-row items-center justify-between gap-4 p-2">
                            <div class="flex items-center gap-4">
                                <div class="relative w-20 h-20 rounded-2xl overflow-hidden shadow-md border-2 border-white bg-white p-2 flex items-center justify-center flex-shrink-0">
                                    <img :src="imagePreview" alt="New Logo Preview" class="max-w-full max-h-full object-contain">
                                </div>
                                <div class="text-left">
                                    <div class="flex items-center gap-2">
                                    ...
                                    <p class="text-[11px] text-emerald-600 font-bold mt-1 flex items-center gap-1">
                                        <i class="ph-bold ph-check-circle"></i> Ready to replace on save
                                    </p>
                                </div>
                            </div>
    ...
    <!-- ============================================================ -->
    <!-- 3D STADIUM BADMINTON SMASH UPLOADING OVERLAY                -->
    <!-- ============================================================ -->
    <div x-show="isUploading" 
        ...
        <div class="bg-gradient-to-b from-[#0a1733] to-[#060e22] border-2 border-[#c9a84c]/40 rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-[0_30px_90px_rgba(0,0,0,0.9)] text-center relative overflow-hidden ring-1 ring-white/20">
            ...
            <div class="relative w-full h-56 sm:h-64 rounded-2xl overflow-hidden shadow-2xl border border-white/15 bg-[#030914] mb-5">
                <canvas id="sponsorEditSmashCanvas" class="w-full h-full block"></canvas>
            ...
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#c9a84c]/20 border border-[#c9a84c]/50 text-[#c9a84c] text-xs font-black uppercase tracking-widest mb-2 shadow-sm">
                ...
                <span x-text="smashStatus">UPDATING SPONSOR...</span>
            </div>

            <!-- High-Contrast Title & Subtitle -->
            <h3 class="text-2xl sm:text-3xl font-black font-display text-white mb-1.5 tracking-tight drop-shadow-sm">Updating Sponsor Logo</h3>
...
<script>
function sponsorEditHandler() {
    return {
        existingLogo: '<?= !empty($sponsor['image_path']) ? BASE_URL . '/uploads/sponsors/' . e($sponsor['image_path']) : '' ?>',
        imagePreview: null,
        fileName: '',
        fileSize: '',
        errorMessage: '',
            ...
            this.$nextTick(() => {
                setTimeout(() => {
                    if (typeof BadmintonSmashEngine !== 'undefined') {
                        this.smashEngine = new BadmintonSmashEngine('sponsorEditSmashCanvas');
