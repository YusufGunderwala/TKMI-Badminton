...
// If match is scheduled, transition to in_progress immediately upon console load so public live feed shows it
if ($match['status'] === MATCH_SCHEDULED) {
    $pdo->prepare("UPDATE matches SET status = 'in_progress', started_at = COALESCE(started_at, NOW()) WHERE id = ?")
        ->execute([$matchId]);
